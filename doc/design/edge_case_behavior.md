...
| Packet        | Not Connected | Connection loss in channel  | Connection loss in message queue | Connection loss in ordering queue | Connection Loss in flight |
| ------------- | ------------- | --------------------------- | --------------------------------| ---- | ---
...
### Clarifications
* A pending completion token returns `CompletionError` if the Session ends before its operation-specific completion event; this table assumes the Session does not end
...
- The channels function as the session message queue (this design was adopted). There are distinct channels for:
    - QoS 0 PUBLISH (no PKID, not subject to server receive maximum) — `publish_qos0_queue_size`
    - QoS 1 PUBLISH + QoS 2 PUBLISH (shared PKID ordering, subject to server receive maximum) — `publish_qos1_qos2_queue_size`
...
- The acknowledgement channel is unbounded so acknowledgement tokens can synchronously submit their default response from `Drop`; acknowledgement ordering remains governed by the internal session state. See [Queueing Considerations](considerations.md) for detailed tradeoffs.
...
- Session message queue size
    ...
    - Can it be configured to be larger than the server receive maximum?
        - Should be allowed - receive maximum only governs the number of in-flight messages
