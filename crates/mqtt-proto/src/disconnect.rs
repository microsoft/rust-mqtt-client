...
mod tests {
    ...
    use super::*;
    use crate::{Packet, byte_str, tests};
    ...
    fn encode_decode_all_the_codes() {
        for code in enum_iterator::all::<DisconnectReasonCode>() {
            ...
            let mut encoding = crate::tests::encode(&packet, crate::ProtocolVersion::V5);
            println!("encoded packet: {encoding:?}");
            let decoded_packet = crate::tests::decode(&mut encoding, crate::ProtocolVersion::V5);
    ...
    fn no_properties_and_no_properties_length() {
        let mut src = tests::create_packet_as_shared(0xE0, &[0x04]);
        let packet = tests::decode(&mut src, ProtocolVersion::V5);
        ...
        let encoded_packet = tests::encode(&packet, ProtocolVersion::V5);
        assert_eq!(encoded_packet.as_ref(), &[0xE0, 0x01, 0x04]);
    }

    #[test]
    fn no_properties_and_explicit_properties_length() {
        let mut src = tests::create_packet_as_shared(0xE0, &[0x04, 0x00]);
        let packet = tests::decode(&mut src, ProtocolVersion::V5);
        assert_matches!(
            &packet,
            Packet::Disconnect(Disconnect {
    ...
    fn some_properties() {
        let mut src =
            tests::create_packet_as_shared(0xE0, &[0x04, 0x05, 0x11, 0xFF, 0xFF, 0xFF, 0xFF]);
        let packet = tests::decode(&mut src, ProtocolVersion::V5);
    ...
    fn zero_session_expiry_interval() {
        let mut src =
            tests::create_packet_as_shared(0xE0, &[0x04, 0x05, 0x11, 0x00, 0x00, 0x00, 0x00]);
        let packet = tests::decode(&mut src, ProtocolVersion::V5);
