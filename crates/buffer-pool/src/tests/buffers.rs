...
use bytes_::{Buf as _, BufMut as _, Bytes, BytesMut};

use crate::{Error, Owned, Shared};
...
impl SharedImpl {
    pub fn from_static(s: &'static [u8]) -> Self {
        SharedImpl(Bytes::from_static(s))
    }

    pub fn copy_from_slice(s: &[u8]) -> Self {
        SharedImpl(Bytes::copy_from_slice(s))
    }
}
